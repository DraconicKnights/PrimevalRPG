...
public class DefaultExecutors {
    ...
    private void execFlagSet(ScriptCommand cmd, ScriptContext ctx) {
        String flag = cmd.getArg("name", null);
        if (flag == null) return;

        var fm = PrimevalRPG.getInstance().getFlagManager();
        for (LivingEntity e : resolveEntities(cmd.targets, ctx.self, ctx.nearby, ctx.event)) {
            fm.setFlag(e.getUniqueId(), flag);
        }
    }
    ...
    private void execFlagClear(ScriptCommand cmd, ScriptContext ctx) {
        ...
        var fm = PrimevalRPG.getInstance().getFlagManager();
        for (LivingEntity e : resolveEntities(cmd.targets, ctx.self, ctx.nearby, ctx.event)) {
            fm.clearFlag(e.getUniqueId(), flag);
        }
    }
     ...
     */
    private Collection<Player> resolvePlayers(
            List<String> targets,
            LivingEntity self,
