...
public class SpawnAroundPlayerRunable extends RunnableCore {
    ...
    private Location randomLocationAround(Location center, int min, int max) {
        ...
        // find the highest solid block at that X,Z
        int surfaceY = world.getHighestBlockYAt(x, z);
        Location loc = new Location(world, x + 0.5, 0, z + 0.5);

        if (rng.nextDouble() < 0.5) {
            int minY = world.getMinHeight() + 5;    // avoid bedrock
            int maxY = surfaceY - 2;               // keep below the surface
            if (maxY > minY) {
                int y = minY + rng.nextInt(maxY - minY + 1);
                loc.setY(y);
            } else {
                // fallback to surface if range invalid
                loc.setY(surfaceY + 1);
            }
        } else {
            // standard surface spawn
            loc.setY(surfaceY + 1);
        }
